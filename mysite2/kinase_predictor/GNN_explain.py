...
class Classifer(nn.Module):
    def __init__(self,
        ...
        self.gcn1_bn = torch.nn.BatchNorm1d(graph_hidden, eps=1e-05, momentum=0.1,
                                            affine=True, track_running_stats=True)
        self.gcn2 = nn.ModuleList(
            [GINConv(apply_func=nn.Linear(graph_hidden, graph_hidden), aggregator_type="sum") for _ in range(depth-1)])
        self.gcn2_bns = nn.ModuleList([torch.nn.BatchNorm1d(graph_hidden, eps=1e-05, momentum=0.1,
                                                            affine=True, track_running_stats=True) for _ in
                                       range(depth-1)])
...
def explain_GNN(smiles,uniprot,mode="group"):
    ...
    del df["ROMol"]
    train_g = [smiles_to_bigraph(smiles, node_featurizer=atom_featurizer, edge_featurizer=bond_featurizer, add_self_loop=False) for smiles in df.canonical_smiles]
    
    print("train_g",train_g[0])
    dataset = DataLoader(train_g, 1, shuffle=False, collate_fn=collate)

    target = int(target_array[uniprot_array==uniprot])
    #模型
    net = Classifer(in_feats=in_feats, graph_hidden=GNN_graph_hidden, depth=GNN_depth,
                    mlp_layers=GNN_mlp_layers,
                    n_classes=n_classes, dropout=GNN_dropout,index=target).to(device)  ### 实例化模型
    print("net",net)
    ...
    if True:
        for index, bg in enumerate(dataset):
            bg = bg.to(device)
            atom_feats=bg.ndata["h"].to(device)
            atom_feats.requires_grad = False
            base_value= net(bg, atom_feats).item()
            ...
            for atom_idx in range(mol.GetNumAtoms()):
                ...
                #屏蔽边
                edge_mask_value=net(bg, atom_feats, mask).item()
            ...
            edge_colors = {_: plt_colors.to_rgba(edge_weight[_]) for _ in range(len(edge_weight))}
            ...
            style="style=\'width: 90%; max-width: "+str(graph_size[0])+"px; height: auto;\'"
            edge_mask = edge_mask[:edge_mask_index]+ style +edge_mask[edge_mask_index:]
